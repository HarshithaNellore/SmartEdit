...
class _ExportScreenState extends State<ExportScreen> {
  ...
  void _startExport() async {
    ...
    try {
      ...
      final outputPath =
          '${appDir.path}/smartcut_export_${DateTime.now().millisecondsSinceEpoch}.$ext';
      ...
      DebugLogger.log(
        'EXPORT',
        'Starting export to $outputPath | Res: ${targetW}x$targetH | FPS: $_fps',
      );
      ...
      for (int i = 0; i < widget.clips!.length; i++) {
        ...
        try {
          if (!kIsWeb && Platform.isWindows) {
            ...
            clipDuration =
                double.tryParse(durationResult.stdout.toString().trim()) ?? 5.0;
          } else {
        ...
        DebugLogger.log(
          'EXPORT',
          'Clip $i: hasAudio=$hasAudio, duration=$clipDuration',
        );

        // Scale to common resolution, padding cleanly to maintain aspect ratio with even dimensions
        filterComplex +=
            '[$i:v]scale=$targetW:$targetH:force_original_aspect_ratio=decrease,pad=$targetW:$targetH:-1:-1:color=black,setsar=1[v$i];';
        ...
        if (hasAudio) {
          ...
          filterComplex +=
              'aevalsrc=0:d=$clipDuration[a$i];'; // Generate exactly `d` seconds of silence
        }
      }
      ...
      bool hasText =
          widget.textOverlays != null && widget.textOverlays!.isNotEmpty;
      filterComplex +=
          'concat=n=${widget.clips!.length}:v=1:a=1[${hasText ? "rawv" : "outv"}][outa]';
      ...
      if (hasText) {
        ...
        for (int i = 0; i < widget.textOverlays!.length; i++) {
          ...
          final safeText = textO.text
              .replaceAll(':', '\\:')
              .replaceAll('\'', '\\\'');
          filterComplex +=
              'drawtext=text=\'$safeText\':fontcolor=white:fontsize=48:x=${textO.offset.dx}:y=${textO.offset.dy}';
      ...
      if (hasBackgroundAudio) {
        if (widget.audioTrimEndMs > 0) {
          ...
          final durationSec =
              (widget.audioTrimEndMs - widget.audioTrimStartMs) / 1000.0;
          args.add('-ss');
        ...
        filterComplex +=
            ';[outa][${widget.clips!.length}:a]amix=inputs=2:duration=first[finala]';
      }
      ...
      if (!kIsWeb && Platform.isWindows) {
        ...
        try {
          ...
          if (exitCode == 0) {
            DebugLogger.log(
              'EXPORT',
              '✅ Export successful! Saved to $outputPath',
            );
            if (mounted) {
              setState(() {
                _exportProgress = 1.0;
          ...
          throw Exception(
            'Failed to run FFmpeg on Windows. Is it installed in your PATH? Error: $e',
          );
        }
      } else {
        ...
        await FFmpegKit.execute(command).then((session) async {
          ...
          if (ReturnCode.isSuccess(returnCode)) {
            ...
            DebugLogger.error(
              'EXPORT',
              'FFmpeg failed with code $returnCode.\nLogs: $logs',
            );
            if (mounted) {
              setState(() {
                _isExporting = false;
              ...
              _showErrorDialog(
                'Export failed during encoding. Please check your video format.',
              );
            }
          }
        });
                    ...
                    child: OutlinedButton.icon(
                      onPressed: () async {
                        ...
                        await Share.shareXFiles([
                          XFile(filePath),
                        ], text: 'Check out my video made with SmartCut! 🎬');
                      },
              ...
              child: Stack(
                ...
                children: [
                  CircularProgressIndicator(
                    ...
                    strokeWidth: 8,
                    backgroundColor: AppTheme.getElevatedColor(context),
